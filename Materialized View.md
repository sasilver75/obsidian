...
- `REFRESH MATERIALIZED VIEW` completely replaces the contents of a materialized view.
	- It's a ==full recomputation==; Postgres might scan millions of rows from source tables, re-aggregate, and write the result. I should say: ==In [[PostgreSQL|Postgres]], there's no incremental/partial refersh, unlike in [[Snowflake]] or [[BigQuery]]==
