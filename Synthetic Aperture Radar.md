...
- ==Sensitive to change==: Comparing two SAR passes of the same area ([[Interferometric Synthetic Aperture Radar|InSAR]]) lets you detect millimeter-scale ground deformation, which is used for earthquake/volcano monitoring and subsidence tracking.
...
# [[Interferometry]] ([[Interferometric Synthetic Aperture Radar|InSAR]]): The Phase Dimension
...
- Four SAR imaging modes
	...
	- ==TOPSAR:== A hybrid that fixes ScanSAR's main problem. Standard ScanSAR has *scalloping*, intensity variations between subswathes because edge areas get fewer pulses than center areas. TOPSAR sweeps the antenna in *bursts* with controlled timing, spreading illumination more evenly across each subswath. The result is a wide swath like ScanSAR but without the scalloping artifacts, and compatible with [[Interferometric Synthetic Aperture Radar|InSAR]], which standard ScanSAR isn't!
