...
- So we wanted to build something to remove ourselves from the loop, so we sprinted after a way to find a way to remove the need for the human to sit in front of their terminal.
...
In symphony there's a rework state where when aPR is proposed and escalated to a human for review:
